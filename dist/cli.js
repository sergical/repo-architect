...
async function runFull(repoRoot, createPr) {
    ...
    if (createPr) {
        ...
        try {
            const result = await createArchPr(repoRoot, writtenFiles.map(f => path.relative(repoRoot, f)), `Full architecture scan: ${analysis.modules.length} modules documented`);
            if (result.prUrl) {
...
async function runIncremental(repoRoot, createPr) {
    ...
    if (createPr) {
        spinner.start('Creating pull request...');
        try {
            const result = await createArchPr(repoRoot, writtenFiles.map(f => path.relative(repoRoot, f)), `Incremental update: ${structuralChanges.length} structural changes`);
            if (result.prUrl) {
                spinner.succeed(`PR created: ${result.prUrl}`);
            }
            else {
